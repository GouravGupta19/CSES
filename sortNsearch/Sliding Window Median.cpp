...
void solve(){
    ...
    vpi v;
    loop(i,0,k) v.emplace_back(a[i],i);
    sortAll(v);
    set<pi> left,right;
    for(int i=0;i<k/2 + (k%2);i++) left.insert(v[i]);
    for(int i=k/2 + (k%2);i<k;i++) right.insert(v[i]);
