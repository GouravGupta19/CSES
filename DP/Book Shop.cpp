...
int f(vi& prices,vi& pages,int idx,int x,vvi& dp){
    int n=prices.sz;
    if(idx==n) rnt 0;
    if(dp[idx][x]!=-1) rnt dp[idx][x];

    int npick=f(prices,pages,idx+1,x,dp);
    int pick=0;
    if(prices[idx]<=x){
        pick= pages[idx]+ f(prices,pages,idx+1,x-prices[idx],dp);
    }

    rtn dp[idx][x] = max(pick,npick);
}

int solve(){
    int n,x; cin>>n>>x;
    vi price(n),pages(n); cin>>price>>pages;
    vvi dp(n+1,vi(x+1,0));

    //rnt f(price,pages,0,x,dp);

    for(int idx=1;idx<=n;idx++){
        for(int tar=0;tar<=x;tar++){
            int pric=price[idx-1];
            int pag=pages[idx-1];
            

            int npick=dp[idx-1][tar];
            int pick=0;
            if(pric<=tar){
                pick= pag + dp[idx-1][tar-pric];
            }

            dp[idx][tar] = max(pick,npick);
        }
    }
