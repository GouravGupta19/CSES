...
int solve(){
    ...
    if(a>b) swap(a,b);
    //a<=b
    //dp[i][j] min ways to cut this rectange of size ixj into square(s)
    vvi dp(a+1,vi(b+1,-1));
    //base case already a rectangle
    loop(i,1,a+1) dp[i][i]=0;

    for(int i=1;i<=a;i++){
        for(int j=1;j<=b;j++){
            if(dp[i][j]!=-1) continue;
            if(in(j,i,a,b) && dp[j][i]!=-1){
                //already computed
                dp[i][j]=dp[j][i];
                continue;
            }
            ...
            int v=1e9;
            for(int k=1;k<j;k++){
                v= min(v, dp[i][k] + dp[i][j-k] );
            }
            //horizontal cutting
            int h=1e9;
            for(int k=1;k<i;k++){
                h= min(h, dp[k][j]+ dp[i-k][j]);
            }
