...
bool dfs(vi adj[],int node,vi& color,vi& vis,int col){
    vis[node]=1;
    color[node]=col;
    for(int neigh:adj[node]){
        if(!vis[neigh]){
            if(!dfs(adj,neigh,color,vis,!col)) rtn 0;
        }else{ //already visited
            if(color[neigh]==color[node]) rtn 0;
        }
    }
    rtn 1;
}
