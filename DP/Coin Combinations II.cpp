...
int f(vi& a,int idx,int tar){
    if(tar==0) rtn 1;
    int n=a.sz;
    if(idx==n-1){
        if(tar%a[idx]==0) rtn tar/a[idx];
        rtn 0;
    }
    int npick=f(a,idx+1,tar);
    int pick=0;
    if(a[idx]<=tar)
        pick= f(a,idx,tar-a[idx]);
...
int solve() {
    ...
    for (int i = 0; i < n; i++) {
        for (int tar = 1; tar <= x; tar++) {
            ...
            if (coin <= tar) {
                pick = dp[i][tar - coin] % mod;
            }

            // Not pick current coin (take result from above row)
            if (i > 0) {
                npick = dp[i - 1][tar] % mod;
            }

            dp[i][tar] = (pick + npick) % mod;
        }
    }
