...
int f(string& s,string& t,int i,int j,vvi& dp){
    if(i<0) rnt j+1;  // Insert all remaining chars of t
    if(j<0) rnt i+1;  // Delete all remaining chars of s
    if(dp[i][j]!=-1) rnt dp[i][j];
    ...
    // Operations
    int add= f(s,t,i,j-1,dp);       // Insert
    int del= f(s,t,i-1,j,dp);       // Delete
    int rep= f(s,t,i-1,j-1,dp);     // Replace

    return dp[i][j]= min({add,del,rep})+1;
}
int solve(){
    string s,t; cin>>s>>t;
    int n=s.sz, m=t.sz; 
