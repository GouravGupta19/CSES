...
template<typename T> ostream &operator<<(ostream& cout,const vector<pair<T,T>> &v){ int n= v.size(); if(!n){ return cout;} cout<<v[0].first<<" "<<v[0].second<<"\n"; for(int i=1;i<n;i++){ cout<<v[i].first<<" "<<v[i].second<<"\n"; } return cout; }
...
void f(int num,int s,int h,int d,vpi& v){
    if(num==0) rtn;
