...
int f(int idx,int n,int tar,vvi& dp){
    if(!tar) rtn 1;
    if(idx==n+1){
        rtn (tar==0);
    }
    if(dp[idx][tar]!=-1) rnt dp[idx][tar];
    int npick= f(idx+1,n,tar,dp)%mod;
    int pick=0;
    if(idx<=tar)
        pick= f(idx+1,n,tar-idx,dp)%mod;

    rtn dp[idx][tar]= (pick+npick)%mod;
}
