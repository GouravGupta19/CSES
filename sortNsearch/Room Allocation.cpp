...
void solve(){
    ...
    for(int i=0;i<n;i++){
        if(pq.empty()){
            rooms[cust[i].ss] = room++;
            pq.push({cust[i].ff.ss, room-1});
            continue;
        }
        if(pq.top().first<cust[i].ff.ff){
            rooms[cust[i].ss]= pq.top().second;
            auto x=pq.top(); pq.pop();
            pq.push({cust[i].ff.ss,x.ss});
        }else{
            rooms[cust[i].ss]= room++;
            pq.push({cust[i].ff.ss,room-1});
        }
