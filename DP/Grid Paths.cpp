...
int f(vvi& mat,int i,int j,vvi& dp){
    int n=mat.sz;
    if(!in(i,j,n,n)) rtn 0;
    if(mat[i][j]) rtn 0;
    if(i==n-1 && j==n-1) rtn 1;
    if(dp[i][j]!=-1) rtn dp[i][j]%mod;

    int d= f(mat,i+1,j,dp)%mod;
    int r= f(mat,i,j+1,dp)%mod;
...
int solve(){
    ...
    if(mat[n-1][n-1]) rtn 0;

    vvi dp(n,vi(n,-1));

    rtn f(mat,0,0,dp)%mod;
}
