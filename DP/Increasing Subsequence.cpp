...
int f(vi& a,int idx,int prev,vvi& dp){
    int n=a.sz;
    if(idx==n) rnt 0;
    if(dp[idx+1][prev+1]!=-1) rtn dp[idx+1][prev+1];
    ...
    //not pick
    int npick=f(a,idx+1,prev,dp);

    rtn dp[idx+1][prev+1]= max(pick,npick);
}

int solve(){
    int n; cin>>n;
    vi a(n); cin>>a;
    //lis ka question hai XD, sirf length print krna hai
    // but constraint is 1e5 ;(
    
    // vvi dp(n+1,vi(n+1,0));
    // //rtn f(a,0,-1,dp);
    // for(int idx=n-1;idx>=0;idx--){
    //     for(int prev=-1;prev<idx;prev++){
    //         int npick= dp[idx+1][prev+1];
    //         int pick=0;
    //         if(prev==-1 || a[prev]<a[idx]){
    ...
    vi temp;
    temp.emplace_back(a[0]);
    loop(i,1,n){
        if(a[i]>temp.back()){
            temp.emplace_back(a[i]);
        }else{
            int idx=lower_bound(all(temp),a[i])- temp.begin();
            temp[idx]=a[i];
        }
    }
