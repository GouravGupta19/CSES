...
void solve(){
    ...
    for(int n=1;n<=N;n++){
        int tot=((n*n)*(n*n-1))/2;
        int attack=(n-1)*(n-2)*2*2;
        outln(tot-attack);
    }
}

/*
total numner of ways to put two pcs ->tot= n^2 * (n^2 -1)
since both are identical so-> tot/=2
now for attack ways for each 2x3 and 3x2 box there are 2 ways to place attacking knights
thus, attack= (n-1)(n-2)*2; coz for each 2x3 and 3x2 box,taking both of them, now multiply by 2 coz each box 
has two ways , attack*=2
*/
